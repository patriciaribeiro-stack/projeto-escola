...
export interface Turma {
  ...
  segmento: Segmento
  serie?: number
  // Idade completa (em anos) exigida até a data de corte pra entrar nessa turma. Opcional —
  // turmas sem essa configuração simplesmente não geram aviso de idade na matrícula.
  idadeMinima?: number
}
...
export interface PushSubscricao {
  id: string
  role: Role
  personaId: string
  endpoint: string
...
// Configuração — uma vez por matéria+série, feita pela coordenação
...
// Registro diário — um por professor, por turma, por matéria, por dia
...
export interface Aluno {
  ...
  // Ficha completa (Fase 2b) — opcionais: alunos cadastrados antes desta fase não têm esses campos
  cpf?: string
  rg?: string
...
export interface Pai {
  ...
  // Ficha completa (Fase 2a) — opcionais: pais seedados antes desta fase não têm esses campos
  cpf?: string
  rg?: string
  orgaoEmissorRg?: string
  dataEmissaoRg?: string
...
export interface DbSchema {
  ...
  unidadesLivro: UnidadeLivro[]
  conteudosDia: ConteudoDia[]
  turmas: Turma[]
  materias: Materia[]
  alunos: Aluno[]
  pais: Pai[]
  ...
  monitoresIntegral: MonitorIntegral[]
  ...
  rotinas: Rotina[]
  licoes: Licao[]
  licaoStatus: LicaoStatus[]
  ocorrencias: Ocorrencia[]
  ocorrenciasGerais: OcorrenciaGeral[]
  eventos: Evento[]
